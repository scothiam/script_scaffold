...
def format_price(val: float | None, currency: str | None = None) -> str:
...
def vote_with_confidence(
    ...
    Args:
        ...
        normalise_fn:    Optional callable(field, value) → str | None that normalises
                         ...
                         Defaults to str(val).strip().lower().
    ...
    if normalise_fn is None:
        def normalise_fn(field, val):  # noqa: F811
    ...
    for extracted in extracted_list:
        for field, val in extracted.items():
            ...
            if field not in votes:
                ...
                norm_to_orig[field] = {}
            votes[field][norm] += 1
            norm_to_orig[field].setdefault(norm, str(val))
